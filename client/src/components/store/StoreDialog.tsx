...
export default function StoreDialog({ open, editingStore, onClose, onSubmit }: Props) {
    ...
    useEffect(() => {
        if (open) {
            ...
            setForm(
                editingStore
                    ? { name: editingStore.name, address: editingStore.address ?? "", companyId: String(editingStore.companyId) }
                    : emptyForm
            );
        }
    }, [open, editingStore]);
    ...
    async function handleSubmit(e: FormEvent) {
        ...
        try {
            ...
            setError(err?.message ?? (editingStore ? "Failed to update store." : "Failed to create store."));
        } finally {
            setSubmitting(false);
        }
    }
        ...
        <div className="dialog-backdrop" onClick={handleBackdropClick}>
            <div className="dialog" role="dialog" aria-modal="true">
                <div className="dialog-header">
                    <h2 className="dialog-title">{editingStore ? "Edit Store" : "New Store"}</h2>
